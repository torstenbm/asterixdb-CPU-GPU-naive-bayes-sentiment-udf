 ...
 */
public class CPUBayesClassifier<T, K> extends Classifier<T, K> {
    ...
    private float featuresProbabilityProduct(Collection<T> features, K category) {
        char[][] featuresFromTweet = features.toArray(new char[0][]);
        char[] tweetFeatureVector = flattenCharArray(featuresFromTweet);
        int[] tweetFeatureIndexVector = makeIndexArray(featuresFromTweet);

        int categoryIndex = (category.equals("positive")) ? 0 : 1;
        float[] probabilityContributions = new float[featuresFromTweet.length];
        char[] featureVector = this.featureVector;
        int[] featureIndexVector = this.featureIndexVector;
        float[] probabilityMatrix = this.probabilityMatrix;

        for (int gid = 0; gid < featuresFromTweet.length; gid++) {
